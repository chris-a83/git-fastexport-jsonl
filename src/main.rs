...
fn main() -> ExitCode {
    ...
    let result = match command {
        "to-jsonl" => to_jsonl(&input, lenient),
        "to-fastexport" => to_fastexport(&input, lenient),
        other => {
...
fn print_usage() {
    eprintln!("usage: fastexport-jsonl <to-jsonl|to-fastexport> [--lenient] [<file>]");
    eprintln!();
...
fn to_jsonl(input: &[u8], lenient: bool) -> Result<Vec<u8>, String> {
    let opts = fastexport::ParseOptions { lenient };
    let events = fastexport::parse(input, &opts).map_err(|e| e.to_string())?;
    let mut out = Vec::new();
    for event in &events {
...
fn to_fastexport(input: &[u8], lenient: bool) -> Result<Vec<u8>, String> {
